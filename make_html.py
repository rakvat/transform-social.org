...
class MetaData:
    ...
    def from_markdown(cls, data):
        return cls(
            ...
            if (summary_field := data.get("summary"))
            else None,
            catchphrases=list(map(str.strip, catchphrases_field[0].split("|")))
            if (catchphrases_field := data.get("catchphrases"))
            else None,
...
ALTERNATE_TEXTS = {
    ...
    "love_yourself_first": {
        "de": "texte/selbstliebe/",
        "en": "texts/love_yourself_first/",
    },
    "selbstliebe": {"de": "texte/selbstliebe/", "en": "texts/love_yourself_first/"},
    "oft_gestellte_fragen_zur_wirtschaft": {
...
for text_path in texts:
    ...
    static_content = article_template.render(
        content=content,
        meta=meta,
        base_url=base_url,
        alternate=ALTERNATE_TEXTS.get(meta.slug),
    )
    (output_path / "texte" / meta.slug).mkdir(parents=True, exist_ok=True)
    with open(
        output_path / "texte" / meta.slug / "index.html", mode="w", encoding="utf-8"
    ) as f:
...
for text_path in texts_en:
    ...
    static_content = article_en_template.render(
        content=content,
        meta=meta,
        base_url=base_url,
        alternate=ALTERNATE_TEXTS.get(meta.slug),
    ...
    with open(
        output_path / "en" / "texts" / meta.slug / "index.html",
        mode="w",
        encoding="utf-8",
    ) as f:
        f.write(static_content)
...
for page in regular_pages:
    ...
    static_content = page_template.render(
        content=content,
        meta=meta,
        base_url=base_url,
        articles=articles,
        alternate=ALTERNATES[page],
    )
...
for page in regular_pages_en:
    with open(
        content_path / "en" / "pages" / f"{page}.md", mode="r", encoding="utf-8"
    ) as f:
        text = f.read()
    content = md.convert(text)
    ...
    articles = (
        text_articles_en
        if page == "texts"
        else bib_articles_en
        if page == "library"
        else None
    )
    static_content = page_en_template.render(
        content=content,
        meta=meta,
        base_url=base_url,
        articles=articles,
    ...
    with open(
        output_path / "en" / page / "index.html", mode="w", encoding="utf-8"
    ) as f:
        f.write(static_content)
...
#################### topics
static_content = topics_template.render(
    tags=tags.items(),
    meta=MetaData.empty(),
    base_url=base_url,
    alternate=ALTERNATES["topics"],
)
...
#################### topics en
static_content = topics_en_template.render(
    tags=tags_en.items(),
    meta=MetaData.empty(),
    base_url=base_url,
    alternate=ALTERNATES["topics"],
...
with open(
    output_path / "en" / "topics" / "index.html", mode="w", encoding="utf-8"
) as f:
    f.write(static_content)
...
sitemap_entries.append(
    SitemapEntry(url=base_url + "en/topics/", date=NOW, freq="daily")
)
for page in regular_pages:
